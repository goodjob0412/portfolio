$(document).ready(function(){
    ...
    if(korean_idx == -1){
        $('.right_item').append(`<h2 class="house_title">해당 숙소가 없습니다.</h2>`)
    }else if(dic_url["type"] == "All"){
        console.log("TIme :: " + dic_url)
        $('.right_item').append(`<h2 class="house_title">'${arr_domestic_korean_All[korean_idx]}' 숙소 ${arr_foreach.length}개</h2>`)
                    ...
                    <a href="#">
                        <div class="house_box_a_div">
                            ...
                            <div class="description_box">
                                ...
                                <div class="house_prices"> &#x20a9; ${(Number(arr_foreach[i].price)).toLocaleString("KO")}</div>
                            </div>
                        </div>
                    </a>
                </div>
            `
            console.log((Number(arr_foreach[i].price)).toLocaleString("KO"))
    ...
    // 검색 바 값 추가
    $('.m_search_box_inner').append(`
        <div class="m_search_locaion">여행지 = ${arr_domestic_korean_All[korean_idx]}</div>
        <div class="m_serach_content">${dic_url["start_date"]} ~ ${dic_url["end_date"]} 인원 ${dic_url["chk_person"]}</div>
    `)


    // 햄버거버튼 추가
    $('.form_inner').append(`
        <div class="m_serach_div">
            <input type="text" name="location" id="location" class="m_search_input" value="${dic_url['location']}">
        </div>
        <div class="m_search_date_box">
            <input value="${dic_url["start_date"]}" type="text" class="datepicker m_search_input" id="start_date" name="start_date">
            <input value="${dic_url["end_date"]}" type="text" class="datepicker m_search_input" id="end_date" name="end_date">
        </div>
        <div class="m_chk_peoples m_search_input">
            <div class="minus_people mouse">-</div>
            <div class="nm_peoles" id="nm_peoles">
                <input type="text" name="chk_person" id="chk_person" value="${dic_url["chk_person"]}">
            </div>
            <div class="plus_people mouse">+</div>
        </div>
        <input type="submit" value="검색" class="m_search_submit">
    `)

    $(document).on('click','.m_search_box_inner', function(){
        $('.m_search_box_inner').animate({
            display:'none'
        },1000)

        $('.m_ham_search_tap').animate({
            left:"0"
        }, 500)
        $('.right_item').css({
            opacity: 0.6
    ...
    $(document).on('click','.m_close_bnt', function(){
        $('.m_search_box_inner').css({
            display:'block'
        })
        $('.m_ham_search_tap').animate({
            left:"100%"
