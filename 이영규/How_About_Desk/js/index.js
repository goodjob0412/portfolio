$(document).ready(function(){
    ...
    $('.banner').css({
        "background":`url(../img/${rand_cnt}_Kv_PC_Light.webp) no-repeat`,
    ...
    // 시작일 종료일 비교하여 종료일이 같거나 뒤에 있을 경우 전송 막기
        ...
        chk_null(start_date, "시작일을 입력해주세요", event)
        chk_null(end_date, "종료일을 입력해주세요", event)
        chk_locate(str_locate, "지역 입력해주세요", event)
        chk_date_vaild(start_date, end_date, "시작일이 종료일 보다 늦습니다", event)


    });

    function chk_locate(str_target , message, event){
        if(str_target == "" || str_target.length == 0){
            alert(message)
    ...
    // 해당 값 널 체크 -> null일 경우 버튼 클릭 못하게
      function chk_null(str_target , message, event){
        ...
        if(str_target == null || str_target.length == 3 ){
            alert(message)
            event.preventDefault();
        }
      }
    ...
    for(let i = 0; i < indi_cnt; i++){
        ...
        for(let j = 0; j < one_view_cnt; j++){
            // 내부 값 추가
            add_html += `
                <a href="#" class="event_img_box">
                <img class="event_img" src="../img/광고${(i * indi_cnt) + j}.jpg" alt="광고${i}">
                </a>
            `
        }
        // 닫을 div 생성
    ...
    function indi_page(){
        ...
        $('.line').eq(event_bang % indi_cnt).addClass('line_active')
    }
    ...
    $('.backgrond_R').on('click', function(){
        slide('-100%', (event_bang+1) % indi_cnt, '100%')
    })
    ...
    $(window).scroll(function(){
        ...
        if(pos_check[0] && s_bot - 100 >= travel_dome_o_top){
