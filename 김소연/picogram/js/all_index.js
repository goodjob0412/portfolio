...
///////////////////////////////////////////////////
    ...
    // if(s_top >= pureal.offsetTop && s_top <= pureal.offsetTop+10){
    //     paths.each(function(i, path) {
    //         m_svg_move(i, path)
    //     });
    //     paths2.each(function(i, polyline) {
    ...
    for(let i=0; i<in_menu.length; i++){
        in_menu[i].addEventListener('click', function(e){
            // 현재 클릭한거
            coming_bang = getElementIndex(document.querySelectorAll('.in_menu'), this);
            
            if(coming_bang != out_bang) {
                if(btn_chk) {
                    clearInterval( interval );
                    btn_chk = false;
                    let tmp_gap = (coming_bang-out_bang<0)?coming_bang-out_bang+menu_count:coming_bang-out_bang;
                    let tmp_count = 0; 
