...
// 메인배너/////////////////////////////////////////////////////////////
    ...
    // $('.new_collection').find('.indi').eq(0).addClass('indi_active');
    ...
    $('.main_banner .indi').click(function(){
        if($('.main_banner .indi_active').index() < $(this).index()) {
            ...
            slide($('.banner'), $('.main_banner'), $(this).index(), '100%', $('.main_banner .indi_active').index(), '-100%', timer);
        }
    ...
    $('.best_area .indi').click(function(){
        if($('.best_area .indi_active').index() < $(this).index()) {
            ...
            slide($('.best_items'), $('.best_area'), $(this).index(), '100%', $('.best_area .indi_active').index(), '-100%', timer/2);
        }
    ...
    $(document).on('click', '.main_banner .next', function(){ 
        ...
        slide($('.banner'), $('.main_banner'), mb_index_no % m_banner_count, '100%', (mb_index_no - 1) % m_banner_count, '-100%', timer);
        mb_index_no+=1; 
    })
    $(document).on('click', '.main_banner .prev', function(){ 
        ...
        mb_index_no-=1; 
        slide($('.banner'), $('.main_banner'), (mb_index_no - 1) % m_banner_count, '-100%', mb_index_no % m_banner_count, '100%', timer);
    })

    $(document).on('click', '.best_area .next', function(){ 
        btn_init(timer/2)

        slide($('.best_items'), $('.best_area'), best_index_no % m_banner_count, '100%', (best_index_no - 1) % m_banner_count, '-100%', timer/2);
        best_index_no+=1; 
    })
    $(document).on('click', '.best_area .prev', function(){ 
        ...
        best_index_no-=1; 
        slide($('.best_items'), $('.best_area'), (best_index_no - 1) % m_banner_count, '-100%', best_index_no % m_banner_count, '100%', timer/2);
    })

    // 버튼 막기
    function btn_init(timer) {
        $('.btn_banner_arrow, .indi, .btn_event_arrow').css({pointerEvents: 'none'});

        setTimeout(() => {
            $('.btn_banner_arrow, .indi, .btn_event_arrow').css({pointerEvents: 'auto'})
        }, timer);
    }
...
//sec////////////////////////////////////////////////////////////////////////////////
    ...
    function act_move_bot(el, this_o_top){
        if(s_bot - 150 >= this_o_top){
            for(let j=0; j<el.length; j++){
                ...
                el.eq(j).addClass('op_active').css({
    ...
    // 768사이즈에서 올라오는 값 
    if(win_wid <= 768){
        for(let j=0; j<$('.look_book .title, .look_book_fw .look_book_list, .look_book_ss .look_book_list').length; j++){
           $('.look_book .title, .look_book_fw .look_book_list, .look_book_ss .look_book_list').addClass('op_active');
           $('.look_book .title, .look_book_fw .look_book_list, .look_book_ss .look_book_list').eq(j).addClass('op_active').css({
                transitionDelay: 0.2 * j +'s'
            });
        }
    }
    ...
    let new_collection = document.getElementsByClassName('new_collection')[0];
    let slide_pan = document.getElementsByClassName('slide_pan')[0];
    let slide_box = document.getElementsByClassName('slide_box')[0];
